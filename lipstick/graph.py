...
def reify(tg: lipstick.schema.TaskGraph) -> networkx.DiGraph:
    ...
    for t, tv  in tg.datatypes.items():
        ...
        for i in tg.interconnects:
            ...
            g.add_edge((t, i.source), (t, i.destination), **{"resource": res, "ratio": i.bandwidth / (tv.size * tv.count)})

            if i.bidirectional:
                g.add_edge((t, i.destination), (t, i.source), **{"resource": res, "ratio": i.bandwidth / (tv.size * tv.count)})

    for a in tg.algorithms.values():
        for i in a.implementations:
            g.add_edge((a.in_type, i.device), (a.out_type, i.device), **{"resource": "d:%s" % i.device, "ratio": i.throughput})
