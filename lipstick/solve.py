...
def optimize_task_graph(tg: networkx.Graph):
    resources = set(j for j in [i[2].get("resource", None) for i in tg.edges(data=True)] if j is not None)
    # non_sink_nodes = [n for n in tg.nodes() if n != "sink" and n != "source"]
    ...
    for (s, d, i) in tg.edges(data=True):
        if "resource" in i:
            flow_variables[(s, d)] = resource_variables[(s, d, i["resource"])] * i["ratio"]
        else:
